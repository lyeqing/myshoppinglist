...
Choose Create an account or Sign in from the welcome page. An active trial offers Keep my list, which registers the same account and retains its list, saved edits and current editor drafts. Registration asks for a display name, email and a password of at least 15 characters (the backend remains authoritative). Forms provide password-manager autocomplete, labelled inputs, pending states and validation/error feedback. Failed submissions retain entered values for correction; cancelling, changing form mode or successful authentication removes the form and password from its state.
