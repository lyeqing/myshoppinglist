...
export interface ListItem { id: number; shoppingListId: number; product: NonNullable<Job["product"]>; quantity: number; notes: string | null; isPurchased: boolean; purchasedDate: string | null; isHidden: boolean; preferredShopId: number | null; addedDate: string; updatedDate: string }
export interface ListItemPage { items: ListItem[]; nextBeforeId: number | null }
export interface ListItemUpdate { quantity: number; notes: string | null; isPurchased: boolean; isHidden: boolean; expectedUpdatedDate: string }
