...
const time = (date: string) => new Date(date).toLocaleString("en-AU", { day: "numeric", month: "short", hour: "numeric", minute: "2-digit" });
function amount(price: Price) { try { return new Intl.NumberFormat("en-AU", { style: "currency", currency: price.currency, currencyDisplay: "code" }).format(price.price); } catch { return `${price.currency} ${price.price.toFixed(2)}`; } }
