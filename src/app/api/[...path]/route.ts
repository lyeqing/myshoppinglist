...
async function forward(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  const allowed = get ? /^(auth\/me|product-import-jobs\/[1-9]\d{0,18}|shopping-lists\/[1-9]\d{0,18}\/imports)$/.test(route)
    : /^(auth\/(trial|logout)|shopping-lists\/[1-9]\d{0,18}\/products\/url)$/.test(route);
  ...
  try {
    ...
    if (get && route.endsWith("/imports")) for (const key of ["beforeId", "pageSize"]) {
    ...
    if (!get) {
      if (route.endsWith("/products/url") && request.headers.get("Content-Type")?.split(";")[0] !== "application/json") return reply(415, "Send a JSON request.");
      ...
      if (reader) {
        ...
        while (true) { const part = await reader.read(); if (part.done) break; size += part.value.length;
          if (size > 4096) { await reader.cancel(); return reply(413, "The request is too large."); }
